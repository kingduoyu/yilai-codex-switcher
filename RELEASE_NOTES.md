...
## v3.2.4（Windows 与 macOS）

- Windows 将旧授权和受管备份文件移入回收站，macOS 移入废纸篓，不再直接永久删除；无法移入时报错并尝试回滚，不使用永久删除兜底。
- 配置写入、会话保留逻辑不变；不自动恢复回收站中的凭据。回滚可能在回收站留下副本，清空回收站将失去该恢复途径。
- 回收站中的文件仍含登录凭据，请勿分享。Windows 未签名，macOS 使用 ad-hoc 签名且未公证。
- 构建、验证结果和最终制品校验值见 releases/v3.2.4/RELEASE-MANIFEST.md。

## v3.2.3（Windows 与 macOS）

- 改用 TOML 结构化重写易来配置，替换整个易来供应商节点，避免旧嵌套请求头和认证参数残留导致配置无效。
- Windows 同步明确启用 image_generation；清理影响本次切换的旧官方认证限制和地址覆盖。
- 保留 MCP、插件、权限、项目设置及其他无关配置值；不修改会话记录或历史数据库。
- 兼容旧 profile 选择：保留其有效设置并合入主配置，移除当前运行时不再接受的旧选择项。
- 配置格式和顺序会规范化，原注释不保留。已有语法错误或重复键会明确报错，原文件不改动。
- Mac 原有 file 凭据存储、旧登录归档清理和失败回滚保持不变。
- Windows 隔离运行时验证了无需官方认证及三个指定模型；Mac 正式双架构构建、隔离自测与 DMG 校验已通过。真实账号下的 Mac 登录及在线生图未验证，不保证所有客户端环境均不弹登录页。
- Windows EXE SHA-256：`2EBF00D0128A81F14718F16C29E9233848F1EFAD88D145072E8D548FF1FD4D7B`。
- macOS DMG SHA-256：`7C5B18583F041749C2AEA92F9B3DE9209DA96427DA5871A6DBDF628A17E21F36`。
- macOS ZIP SHA-256：`72790A73562798C49A487C92439DD09E07AF7C8EC4F468C83339F1A5837CE1D2`。
- 正式制品与完整验证记录见 [RELEASE-MANIFEST](releases/v3.2.3/RELEASE-MANIFEST.md)。
