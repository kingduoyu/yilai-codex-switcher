...
try {
    ...
    $compileArgs = @('-std=c++17','-O2','-DUNICODE','-D_UNICODE','-DWINVER=0x0A00','-D_WIN32_WINNT=0x0A00','-municode','-mwindows','-static','-static-libgcc','-static-libstdc++','-Wno-deprecated-literal-operator',
        '-I',"$root/Sources/ConfigRewrite/include",'-I',"$root/Sources/HistorySync/include",'-I',"$root/Sources/Diagnostics/include",'-I',"$root/Sources/OperationGuard/include",'App.cpp','Platform.cpp',"$root/Sources/ConfigRewrite/config_rewrite.cpp", "$root/Sources/HistorySync/history_sync.cpp", "$root/Sources/Diagnostics/diagnostics.cpp","$root/Sources/OperationGuard/operation_guard.cpp",$sqlite,"$output/resources.o",'-o',"$output/YilaiCodexSwitcher.exe",'-lcomctl32','-lshell32','-lole32','-luuid','-lgdi32','-luser32','-ladvapi32','-lwindowscodecs')
