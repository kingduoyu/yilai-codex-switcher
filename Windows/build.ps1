...
if(!(Test-Path $sqlite) -or (Get-Item "$root/Sources/HistoryRepair/vendor/sqlite3.c").LastWriteTimeUtc -gt (Get-Item $sqlite).LastWriteTimeUtc) {
    & "$bin/x86_64-w64-mingw32-clang.exe" -O2 -c "$root/Sources/HistoryRepair/vendor/sqlite3.c" -o $sqlite
...
try {
    ...
    $compileArgs = @('-std=c++17','-O2','-DUNICODE','-D_UNICODE','-DWINVER=0x0A00','-D_WIN32_WINNT=0x0A00','-municode','-mwindows','-static','-static-libgcc','-static-libstdc++','-Wno-deprecated-literal-operator',
        '-I',"$root/Sources/ConfigRewrite/include",'-I',"$root/Sources/HistoryRepair/include",'-I',"$root/Sources/Diagnostics/include",'-I',"$root/Sources/OperationGuard/include",'-I',"$root/Sources/ConfigSources/include",'App.cpp','Platform.cpp',"$root/Sources/ConfigRewrite/config_rewrite.cpp", "$root/Sources/HistoryRepair/history_repair.cpp", $sqlite, "$root/Sources/Diagnostics/diagnostics.cpp","$root/Sources/OperationGuard/operation_guard.cpp","$root/Sources/ConfigSources/config_sources.cpp","$root/Sources/ConfigSources/runtime_probe.cpp","$output/resources.o",'-o',"$output/YilaiCodexSwitcher.exe",'-lcomctl32','-lshell32','-lole32','-luuid','-lgdi32','-luser32','-ladvapi32','-lwindowscodecs')
