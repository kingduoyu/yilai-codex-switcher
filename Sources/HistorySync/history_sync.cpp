...
std::string normalize(const std::string &config) {
  char *error = nullptr;
  char *result =
      yilai_apply_config(config.c_str(), "", YILAI_UNIFY_HISTORY, &error);
  std::string diagnostic = error ? error : "Cannot unify connection";
  if (error)
    yilai_config_free(error);
  ensure(result != nullptr, diagnostic);
