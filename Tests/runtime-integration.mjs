...
try{
 ...
 assert.equal(failedSwitch.status,0);assert(!failedSwitch.stderr.includes(privateFragment));assert(!failedSwitch.stderr.includes('sk-isolated-test-only'));
 assert.equal(await readFile(path.join(home,'config.toml'),'utf8'),configBeforeFailure);await assert.rejects(access(path.join(home,'auth.json')));
 ...
 assert(logText.includes('sync_history')&&logText.includes('history_warning')&&logText.includes('API configured; history incomplete'));
 facts.passed=['failed automatic sync warns without undoing API configuration or leaking credentials/history','one-click API config deletes auth and automatically syncs legacy history','account/read confirms no official login required after auth deletion','custom provider preserves visible local history across credential changes','native image_gen.imagegen advertised','internal image-only operation preserves model/catalog/auth','enhance idempotent','native model/list gpt-6-astra','real thread written by app-server','legacy provider sync preserves content','native list/read/resume after sync','new message retained by undo','custom image header reaches local mock'];facts.requestTools=requests.filter(x=>x.url.endsWith('/responses')).map(x=>(x.body.tools??[]).map(t=>t.name??t.type));facts.status='passed';
